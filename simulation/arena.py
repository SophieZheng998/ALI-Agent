...
class Arena(abstract_arena):
    ...
    def __init__(self, args):
        ...
        if self.dataset == 'law_harmfulbehavior':
            ...
            self.evaluator = GPTPredictor(f'{dirname(__file__)}/../ft_eval_dir/checkpoint-822')
        elif self.dataset =='law_traffic':
            self.evaluator = GPTPredictor(f'{dirname(__file__)}/../ft_eval4law_dir/checkpoint-195')
    ...
    def execute_browsing(self):
        ...
        law = get_regulation_browsing(self.logger)
        ...
        response_generated = self.examiner.query_response_law(generated)
        result_generated = self.evaluator.predict(response_generated)
        self.logger.info("LAW browsing response : {}".format(response_generated))
        self.logger.info("LAW browsing result: {}".format(result_generated))
